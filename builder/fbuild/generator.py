...
def xbuild(config: Dict[str, Dict[str, Any]], x_out_dir: Path, info: Info) -> None:
    """Build `Fuchsia` cursor theme for only `X11`(UNIX) platform.

    :param config: `Fuchsia` configuration.
    :type config: ``Dict``
...
def wbuild(config: Dict[str, Dict[str, Any]], win_out_dir: Path, info: Info) -> None:
    """Build `Fuchsia` cursor theme for only `Windows` platforms.
...
def build(
    ...
    """Build `Fuchsia` cursor theme for `X11` & `Windows` platforms.
